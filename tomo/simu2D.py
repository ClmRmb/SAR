...
class Scene:
    ...
    sensors=None,B=40e6,f0=1e9,c=3e8,mean_height=1000,dy=None,flat_ref=False) -> None:
        ...
        if sensors:
            ...
            self.S[0] = np.linspace(0,self.N-1,self.N)*50
            self.S[1] = self.H + np.linspace(0,self.N-1,self.N)*15
...
class SARImage:
    def __init__(self,topo,geo) -> None:
       ...
       self.I = np.zeros((geo.N,geo.Nr),dtype='complex128')
       self.Ic = np.zeros_like(self.I,dtype='complex128')
       self.Ii = np.zeros_like(self.I,dtype='complex128')
       self.Cov = np.zeros((geo.N,geo.N,geo.Nr),dtype='complex128')
       self.N = geo.N
       self.Nr = geo.Nr
